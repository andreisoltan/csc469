...
static void announce_server_ready(char *local_host_name) {
  ...
  bzero(output, maxlen);
  snprintf(output, maxlen, "scp %s cdf.toronto.edu:/u/csc469h/fall/public_html/chatserver.txt",template);
...
int create_room(char *room_name) {
    ...
    /* 
     * go through the room list and see whether the room with same name exists 
     */
...
init_server(){
    ...
    ret = gethostname(local_host_name, MAX_HOST_NAME_LEN);
    ...
    announce_server_ready(local_host_name);
#endif
